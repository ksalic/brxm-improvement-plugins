...
//noinspection Duplicates https://issues.onehippo.com/browse/CMS-11961
public class DocumentListFilter implements IClusterable {
    ...
    private static class FilterDefinition implements IClusterable {
        ...
        FilterDefinition(String state, String path, String parent, String child, String subChild, String targetState, boolean targetDisplay, String targetName) {
            this.state = state;
            this.path = path;
            this.parent = parent;
            this.child = child;
        ...
        boolean match(String currentState, Node node) throws RepositoryException {
            ...
            if (!child.isEmpty() && !node.isNodeType(child) && subChild.isEmpty()) {
                return false;
            }
            if (!child.isEmpty() && node.isNodeType(child) && !subChild.isEmpty() && !hadSubChildrenOfType(node, subChild)) {
                return false;
            }
            return true;
        }

        public boolean hadSubChildrenOfType(Node node, String subchildType) throws RepositoryException {
            boolean hasSubChildOfType = false;
            if (node != null) {
                NodeIterator it = node.getNodes();
                while (it.hasNext()) {
                    Node subChild = it.nextNode();
                    if (subChild != null && subChild.isNodeType(subchildType)) {
                        hasSubChildOfType = true;
                        break;
                    }

                }
            }
            return hasSubChildOfType;
        }
    }
}
